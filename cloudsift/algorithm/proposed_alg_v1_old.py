...
class ReconstructionImageCloudLabeler(BaseCloudLabeler):
    def __init__(
            self,
    ...
    def request_labels(
            ...
            unseen_orig_labels: IntArrayLike1D) -> list[tuple[bool, Union[int, None]]]:
...
class CloudComputer(BaseCloudComputer):
    def __init__(
        ...
        if labeling_method == LabelingMethod.AUTOMATIC:
            ...
            label_allocator = NotImplemented
    ...
    def _train_policy(
        ...
        # Stage 1: train classifier, encoder and decoder networks
        print("Stage 1: Training classifier, encoder and decoder networks")
        ...
        for i in range(self.max_num_main_epochs_per_round):
            ...
            for batch in pbar:
                ...
                if batch['compressed'] is not None:
                    inputs_latents, imgs_decoded, labeling_successes, labels = batch['compressed']
                    inputs_latents = inputs_latents.to(self.device)
                ...
                pbar.set_description(
                    "Avg. Loss: {:.4f}, Components: {}, Acc.: {:.4f}".format(
                        training_loss,
                        ", ".join(["{:.4f}".format(n) for n in loss_components]),
                        training_accuracy))

            if training_accuracy >= self.round_early_stop_acc:
                break
            scheduler.step(training_loss)
            pbar.close()
        stage1_training_accuracy = training_accuracy
        stage1_training_loss = training_loss

        # Stage 2: train importance and compression action networks
        print("Stage 2: Training importance and compression action networks")
        ...
        for compressed in [True, False]:
            ...
            for batch in pbar:
                if batch['compressed'] is not None:
                    _, imgs_decoded, labeling_successes, labels = batch['compressed']
                    imgs_decoded = imgs_decoded.to(self.device)
                    labels = labels.to(self.device)
        ...
        importance_loss_min = masked_losses.min().to(self.device)
        importance_loss_max = masked_losses.max().to(self.device)
        ...
        for i in range(self.num_action_train_epochs_per_round):
            print("Epoch {}".format(i + 1))
        ...
        stage2_training_loss = training_loss

        training_stats = {
            'stage1_acc': stage1_training_accuracy,
            'stage1_loss': stage1_training_loss,
            'stage2_loss': stage2_training_loss
        }
    ...
    def _add_data_point(
        ...
        labeling_success, label = labeling_out
