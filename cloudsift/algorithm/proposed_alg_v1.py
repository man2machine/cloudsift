...
class ReconstructionImageCloudLabeler(BaseCloudLabeler[int]):
    ...
    def request_labels(
        ...
        out = [
            LabelingResult(labeling_success=bool(v), label=(int(l) if v else None))
            for (l, v) in zip(labels, labeling_successes)]
    ...
    def __init__(
        ...
        if labeling_method == LabelingMethod.AUTOMATIC:
            ...
            label_allocator = ManualCategoricalImageCloudLabeler(
                ...
                img_display_size=(IMG_DATASET_TO_IMG_SHAPE[dataset_type][1] * 3))
        ...
        for batch in pbar:
            ...
            if batch['uncompressed'] is not None:
                ...
                with torch.set_grad_enabled(True):
                    ...
                    labeling_successes = \
                        labeling_successes.float()
                    compression_losses = F.binary_cross_entropy_with_logits(
                        outputs['compress_outs'][:, 0],
                        labeling_successes,
                        reduction='none')
                    imp_loss_targets = classification_losses.detach()
                    imp_loss_targets = \
                    ...
                    importance_loss = importance_losses.mean()
                    compression_loss = compression_losses.mean()
